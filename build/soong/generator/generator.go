...
// Copyright (C) 2018 The titaniumOS Project
//
...
func init() {
	android.RegisterModuleType("titanium_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = titaniumExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = titaniumExpandVariables(ctx, dep_file)
	...
	cmd := titaniumExpandVariables(ctx, String(g.properties.Cmd))
