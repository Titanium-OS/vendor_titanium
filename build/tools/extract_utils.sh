...
# Copyright (C) 2017-2019 TitaniumOS
#
...
function setup_vendor() {
    ...
    export TITANIUM_ROOT="$3"
    if [ ! -d "$TITANIUM_ROOT" ]; then
        echo "\$TITANIUM_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$TITANIUM_ROOT/$OUTDIR" ]; then
        mkdir -p "$TITANIUM_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$TITANIUM_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$TITANIUM_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$TITANIUM_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$TITANIUM_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_blueprint_header() {
    ...
    if [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
        printf " * Copyright (C) $YEAR TitaniumOS\n" >> $1
    elif [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -le 2019 ]; then
        printf " * Copyright (C) 2019-$YEAR TitaniumOS\n" >> $1
    else
        printf " * Copyright (C) $BLUEPRINT_INITIAL_COPYRIGHT_YEAR-$YEAR TitaniumOS\n" >> $1
    fi
...
function write_makefile_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2017 ]; then
            printf "# Copyright (C) 2017 TitaniumOS\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
            printf "# Copyright (C) $YEAR TitaniumOS\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -le 2017 ]; then
            printf "# Copyright (C) 2017-$YEAR TitaniumOS\n" >> $1
        else
            printf "# Copyright (C) $INITIAL_COPYRIGHT_YEAR-$YEAR TitaniumOS\n" >> $1
        fi
    else
        printf "# Copyright (C) $YEAR TitaniumOS\n" > $1
    fi
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$TITANIUM_ROOT"/prebuilts/tools-titanium/common/smali/baksmali.jar
        export SMALIJAR="$TITANIUM_ROOT"/prebuilts/tools-titanium/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$TITANIUM_ROOT"/prebuilts/tools-titanium/${HOST}-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$TITANIUM_ROOT"/prebuilts/tools-titanium/${HOST}-x86/bin/compact_dex_converter
    fi
    ...
    if [ ! -f "$TITANIUM_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$TITANIUM_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$TITANIUM_ROOT"/"$OUTDIR"/proprietary
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$TITANIUM_ROOT"/vendor/titanium/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$TITANIUM_ROOT"/"$OUTDIR"/radio
