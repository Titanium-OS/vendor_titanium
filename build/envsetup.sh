...
- titaniumgerrit:   A Git wrapper that fetches/pushes patch from/to TitaniumOS Gerrit Review.
- titaniumrebase:   Rebase a Gerrit change and push it again.
- titaniumremote:   Add git remote for TitaniumOS Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for TitaniumOS Github.
...
function titaniumremote()
{
    ...
    git remote rm titanium 2> /dev/null
    ...
    then
        ...
        local PFX="TitaniumOS/"
    else
        local PROJECT=$REMOTE
    fi

    local TITANIUM_USER=$(git config --get review.review.titanium.org.username)
    if [ -z "$TITANIUM_USER" ]
    then
        git remote add titanium ssh://review.titanium.org:29418/$PFX$PROJECT
    else
        git remote add titanium ssh://$TITANIUM_USER@review.titanium.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'titanium' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/TitaniumOS/$PROJECT
...
function makerecipe() {
    ...
    then
        pwd
        titaniumremote
        git push titanium HEAD:refs/heads/'$1'
    fi
    '
}

function titaniumgerrit() {
    ...
    local user=`git config --get review.review.titanium.org.username`
...
EOF
            ...
            case $1 in
                ...
                changes|for)
                    if [ "$FUNCNAME" = "titaniumgerrit" ]; then
...
function titaniumrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "TitaniumOS Gerrit Rebase Usage: "
        echo "      titaniumrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.titanium.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function cmka() {
    if [ ! -z "$1" ]; then
        for i in "$@"; do
            case $i in
                titanium|otapackage|bacon|systemimage)
                    mka installclean
                    mka $i
                    ;;
                *)
...
function repopick() {
    T=$(gettop)
    $T/vendor/titanium/build/tools/repopick.py $@
}
